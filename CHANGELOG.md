...
- 趋势区间选择器重做：原生 <select> 下拉（移动端样式出戏）替换为 DSW 风格分段
  控件（pill 组，即点即切、选中态浮起、窄屏横向滚动不换行）。
- 修复同步 bug：此前在下拉选择「昨日/近30天」时，顶部摘要卡片的选中高亮不会跟
  随取消；现在切区间做全量重渲染，卡片/分段控件/图表三处始终一致。
- 冒烟测试增至 23 项断言，新增分段控件与卡片同步回归守卫。

## 0.2.2 (2026-08-19)

- UI 全面重做（DSW 原生风格）：面板/卡片/图表/表格/弹窗全部改用 GUI 设计令牌
  （--dsw-alias-*，bg-layer/border/label/brand/state/shadow），自动适配明暗主题，
  移除硬编码纸片配色与手绘橘猫插画。
- 侧边栏入口换成仪表盘线性图标；胶囊改为令牌化小圆角 chip。
- 摘要卡片：圆角 12px、悬浮微抬升、选中态品牌色描边 + 角标圆点（纯 CSS，替换原
  内联样式）；额度条改渐变填充、warn/crit 阈值色用 state-warn/error。
- 趋势图：平滑贝塞尔曲线替代折线、品牌色渐变面积、网格/刻度/气泡全部令牌化，
  峰值点 warn 色发光；模型表改无边框行 + 品牌色占比条。
- 令牌弹窗：主按钮品牌色填充、输入框聚焦描边、token 化背景。
- 移动端改底部 sheet（顶部把手 + 上滑动画 + safe-area）。
- 新增 tests/render-smoke.mjs：jsdom 渲染冒烟测试（20 项断言，覆盖 0.2.0 崩溃
  路径：绑定态图表渲染、令牌弹窗、未绑定降级、卸载清理）。
