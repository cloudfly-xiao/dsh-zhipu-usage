...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      '.dsh-zpu-select{font-size:12px;padding:4px 26px 4px 10px;border:1px solid var(--zpu-line);border-radius:8px;background:var(--zpu-paper);color:var(--zpu-ink);font-family:inherit;font-weight:500;cursor:pointer;-webkit-appearance:none;appearance:none;-webkit-tap-highlight-color:transparent;background-image:url("data:image/svg+xml;utf8,<svg xmlns=\'http://www.w3.org/2000/svg\' width=\'10\' height=\'6\' viewBox=\'0 0 10 6\'><path d=\'M1 1l4 4 4-4\' fill=\'none\' stroke=\'%2380868f\' stroke-width=\'1.6\' stroke-linecap=\'round\'/></svg>");background-repeat:no-repeat;background-position:right 9px center}',
      '.dsh-zpu-select:hover{border-color:var(--dsw-alias-border-l3,#c9cdd4)}',
    ...
    function renderUsageDetail(cu) {
      ...
      const select = el('select', 'dsh-zpu-select')
      ...
      const head = el('div', '', '')
      head.style.cssText = 'display:flex;align-items:center;margin:14px 0 0;font-size:13px;font-weight:600;color:var(--zpu-ink)'
