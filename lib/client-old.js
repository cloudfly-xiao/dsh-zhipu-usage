...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      '.dsh-zpu-panel{--zpu-ink:var(--dsw-alias-label-primary,#2f2f2f);--zpu-ink-soft:var(--dsw-alias-label-secondary,#6b6b6b);--zpu-ink-faint:var(--dsw-alias-label-tertiary,#a8a8a8);--zpu-paper:var(--dsw-alias-bg-layer-2,#ffffff);--zpu-paper-raised:var(--dsw-alias-bg-layer-1,#fbfbfa);--zpu-track:var(--dsw-alias-interactive-bg-hover,rgba(130,145,165,.14));--zpu-cat:#f5c56b;--zpu-cat-deep:#e8a94e;--zpu-blue:#a9c3d8;--zpu-blue-faint:#dbe7f0;--zpu-accent:var(--zpu-cat-deep);--zpu-mark:rgba(245,197,107,.3);--zpu-ok:var(--zpu-blue)}',
      ...
      '.dsh-zpu-panel{width:min(580px,100%);max-height:calc(100vh - 48px);max-height:calc(100dvh - 48px);overflow-y:auto;background:var(--zpu-paper);color:var(--zpu-ink);border:1.5px solid var(--zpu-ink);border-radius:10px;box-shadow:0 10px 32px rgba(50,50,50,.14);padding:20px 20px 34px;font-size:13px;overscroll-behavior:contain;position:relative;font-family:ui-rounded,"Segoe UI Rounded",system-ui,sans-serif}',
      ...
      '@media (max-width:480px){.dsh-zpu-cards{gap:7px}.dsh-zpu-card{padding:10px 8px;border-radius:7px}.dsh-zpu-cardName{font-size:10px}.dsh-zpu-cardValue{font-size:16px}.dsh-zpu-cardMeta{font-size:9.5px;line-height:1.45}}',
      '.dsh-zpu-card{border:1.2px solid var(--zpu-ink);border-radius:8px;padding:12px 14px;background:var(--zpu-paper-raised);transition:border-color .15s,background .15s;cursor:pointer;position:relative}',
      '.dsh-zpu-card:hover{border-color:var(--zpu-cat-deep)}',
      '.dsh-zpu-card[data-active]{background:var(--zpu-blue-faint);border-color:var(--zpu-ink)}',
      ...
      '.dsh-zpu-cardName{font-size:10.5px;color:var(--zpu-ink-soft);letter-spacing:.06em;white-space:nowrap}',
      '.dsh-zpu-cardValue{font-size:19px;font-weight:700;font-variant-numeric:tabular-nums;margin:4px 0 3px;color:var(--zpu-ink);overflow-wrap:anywhere}',
      '.dsh-zpu-card[data-active] .dsh-zpu-cardValue{color:var(--zpu-cat-deep)}',
      '.dsh-zpu-cardMeta{font-size:10.5px;color:var(--zpu-ink-soft);line-height:1.55}',
    ...
    function renderUsageDetail(cu) {
      ...
      if (allModels.length > FOLD) {
        ...
        moreBtn.type = 'button'
        moreBtn.style.cssText = 'display:block;width:100%;margin-top:10px;margin-bottom:6px;text-align:center;color:var(--zpu-ink);font-weight:700'
    ...
    function openTokenDialog() {
      ...
      const box = el('div')
      box.style.cssText = 'width:min(420px,92vw);background:#ffffff;border:1.5px solid #2f2f2f;border-radius:10px;padding:16px;box-shadow:0 12px 36px rgba(50,50,50,.18)'
      const title = el('div', '', '更新令牌')
      title.style.cssText = 'font-size:15px;font-weight:800;margin-bottom:6px;color:var(--zpu-ink)'
      ...
      ta.style.cssText = 'width:100%;box-sizing:border-box;padding:8px 10px;border:1.2px solid #2f2f2f;border-radius:7px;background:#fbfbfa;color:#2f2f2f;font-size:12px;font-family:ui-monospace,monospace;resize:vertical;line-height:1.5'
      const msg = el('div')
