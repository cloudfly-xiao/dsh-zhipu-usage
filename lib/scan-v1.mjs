...
function recordsToEntry(records, mtimeMs, size) {
  ...
  return { mtimeMs, size, firstTs, lastTs, dayModel: [...dayModel.values()], hours: hourList }
}
...
function aggregateEntries(entries, now = Date.now()) {
  ...
  return { totals, byDay: byDayList, byHour, byModel: [...byModel.values()].sort((a, b) => b.total - a.total), firstTs, lastTs }
}
 ...
 * @param options { ledgerPath } - persistent ledger location; null disables;
 *   defaults to "<sessions parent>/zhipu-usage-ledger.json".
 */
  ...
  for (const workspace of workspaces) {
    ...
    for (const sessionDir of sessionDirs) {
      ...
      for (const name of ['session.jsonl.zstd', 'session.jsonl']) {
        const candidate = join(root, workspace.name, sessionDir.name, name)
        try { await stat(candidate); file = candidate } catch { /* try next */ }
        if (file !== null) break
      }
